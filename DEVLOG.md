...
- 미구현 메모: 힘/하이브리드는 시작자세 기준 단일 벽이라 **위치 프로그램 구동 비해당**
  (force=힘 regulate / hybrid=직선 한 획). 경로 추종(컨투어) 확장은 별도 과제 → 아래 착수.

### 🌀 하이브리드 컨투어 추종 Step 1 — 곡면 + 경로 (branch `hybrid-contour`)

하이브리드를 "티칭 경로를 따라가며 표면을 일정 힘으로 누르기"(폴리싱/디버링/용접)로 확장.
**MR §11.6 그대로**: 구속 A(θ)가 위치종속이라 곡면은 접촉점 법선 n(p)만 매 스텝 달라질 뿐
제어식(투영 P, 식 11.60/11.61)은 동일. 곡면 모델은 해석적(원기둥/구 = 중학교 기하)이라 교재
밖 추가 아님(임의 메시+센서 법선추정만 교재 밖).

- **`sim._make_surface`**: 평면/원기둥/구 공용 `surface(p)→(n 외향법선, δ 침투, p_proj 표면투영)`.
  원기둥 n=축에서 반경방향·구 n=중심에서 방향. `_surface_contact` penalty 접촉.
- **`_run_hybrid` 확장**: 티칭 waypoint(distinct ≥2)면 표면 투영 경로를 §9 quintic 으로
  접선 추종(컨투어), 1점이면 기존 직선 한 획(하위호환). rhs 가 매 스텝 `n(p)`로 투영 P 재계산
  (Level 1: 공구 자세 R0 고정). `tcp_des`(목표 경로) 결과에 추가. `_prep_waypoints` 단일포즈
  복제 → 연속중복 dedup 으로 회귀 수정.
- **검증**: 평면 1획 회귀 무손상(20.00N·80mm·1.0mm). 원기둥 20.00±0.03N·77mm 호·1.1mm,
  구 20.00±0.02N·1.4mm, 발산 없음. `demos/contour_demo.py`(원기둥 48° 스윕, 단면 궤적 플롯).
- 남은 것: **Level 2**(공구 자세가 법선 따라감 — 곡면다움, R_d(t)만 추가, §11.6 안) →
  **프론트**(스키마 shape/center/radius + UI + 3D 곡면 렌더 + 프로그램으로 hybrid 실행).
